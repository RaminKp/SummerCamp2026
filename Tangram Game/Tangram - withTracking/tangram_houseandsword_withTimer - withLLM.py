...
# Offline child-friendly narration cache. Variations were pre-generated by
# generate_narration_cache.py with a local LLM; the runtime just does a
# random.choice() per spoken line. If narration_cache.json is missing or a
# specific key is absent, Narration falls back to its hardcoded defaults so
# the game keeps running without any LLM call at runtime.
from narration import Narration
NARRATION = Narration("narration_cache.json")
...
def build_feedback(statuses: Dict[int, str], newly_correct: List[int]
    ...
    """Return a list of (expression, gesture, sentence). All spoken lines come
    from the offline narration cache (with hardcoded fallbacks); the structure
    and singular/plural split is the same as before."""
    ...
    if not correct and not rotate and not misplaced:
        seg.append(("neutral", "shrug", NARRATION.pick("empty_board")))
        return seg

    if newly_correct:
        names = _join([_the(p) for p in newly_correct]).capitalize()
        if len(newly_correct) >= 2:
            seg.append(("happy", "celebrate",
                        NARRATION.pick("newly_correct_multi", names=names)))
        else:
            seg.append(("happy", "nod",
                        NARRATION.pick("newly_correct_single", names=names)))

    if len(correct) >= 2 and not newly_correct:
        seg.append(("happy", "excited",
                    NARRATION.pick("progress", n=len(correct))))

    if rotate:
        names = _join([_the(p) for p in rotate]).capitalize()
        key = "rotate_single" if len(rotate) == 1 else "rotate_multi"
        seg.append(("surprise", "point", NARRATION.pick(key, names=names)))

    if misplaced:
        names = _join([_the(p) for p in misplaced]).capitalize()
        key = "misplaced_single" if len(misplaced) == 1 else "misplaced_multi"
        seg.append(("neutral", "shrug", NARRATION.pick(key, names=names)))
    return seg


# Maps hint statuses to their narration cache keys (filled at speak-time with
# the piece's label, e.g. "dark blue triangle").
_HINT_KEY = {
    ST_MISSING:   "hint_missing",
    ST_MISPLACED: "hint_misplaced",
    ST_ROTATE:    "hint_rotate",
}
...
def handle_hint(qt, state, detections, board_reg, model):
    if not board_reg.is_valid:
        ...
        qt.say(NARRATION.pick("hint_no_board"))
        return
    statuses = evaluate_board(detections, board_reg, model, state.placed)
    for st in (ST_MISSING, ST_MISPLACED, ST_ROTATE):
        ...
        if cand:
            ...
            qt.say(NARRATION.pick(_HINT_KEY[st],
                                  label=PIECES[cand[0]]["label"]))
            qt.reset_pose()
            return
    qt.express("happy"); qt.gesture("nod")
    qt.say(NARRATION.pick("hint_all_good"))
...
def _on_complete(qt, display, win):
    ...
    qt.say(NARRATION.pick("complete_intro", name=name))
    qt.gesture("dance")
...
# Fixed cycle of (expression, gesture) pairs - the spoken line itself is
# pulled fresh from the narration cache on each nudge, so children hear
# different wording each time even though the gesture rotates.
_INACTIVITY_GESTURES = [
    ("neutral", "nod"),
    ("surprise", "point"),
    ("happy", "wave"),
    ("thinking", "shrug"),
...
def build_timer_milestones(total_seconds: float, final_warning_seconds: float = 20.0):
    ...
    for m in range(int(total // 60), 0, -1):
        ...
        if t >= total:                       # start is announced on its own
            continue
        if m == 1:
            msg = NARRATION.pick("time_left_one_minute")
            expr, gest = "surprise", "excited"
        else:
            msg = NARRATION.pick("time_left_minutes",
                                 n=_num_word(m).capitalize())
    ...
    if 0 < fw < total and all(abs(fw - d["t"]) > 1e-6 for d in ms):
        ...
        ms.append({"t": fw, "expr": "surprise", "gest": "excited",
                   "msg": NARRATION.pick("time_left_final", seconds=secs)})
    ...
    try:
        ...
        while True:
            ...
            if board_reg.is_valid and not welcomed:
                ...
                qt.express("happy")
                qt.say_with_gesture(NARRATION.pick("welcome", name=name),
                                    "wave")
                time.sleep(0.1)
                qt.say_with_gesture(
                    NARRATION.pick("instructions", button_word=button_word),
                    "point")
                ...
                if timer_total > 0:
                    ...
                    qt.express("surprise")
                    qt.say_with_gesture(
                        NARRATION.pick("timer_intro",
                                       duration=_spoken_duration(timer_total),
                                       name=name),
                        "excited")
                else:
                    milestones = []
            ...
            if timer_total > 0 and game_start_time is not None:
                ...
                if (not timed_out) and time_left <= 0:
                    ...
                    if not round_scored:
                        round_scored = True
                        qt.express("neutral")
                        qt.say_with_gesture(NARRATION.pick("time_up"), "nod")
                        qt.reset_pose()
            ...
            if buzzed and (time.time() - last_press) > BUZZER_COOLDOWN:
                ...
                if not board_reg.is_valid:
                    qt.express("neutral"); qt.gesture("shrug")
                    qt.say(NARRATION.pick("buzz_no_board"))
                    continue
            ...
            if (time.time() - last_activity) > INACTIVITY_TIMEOUT and not inactivity_nudged \
                    and welcomed:
                expr, gest = _INACTIVITY_GESTURES[
                    nudge_idx % len(_INACTIVITY_GESTURES)]
                msg = NARRATION.pick("nudge")
                nudge_idx += 1
            ...
            if state.is_complete and not round_scored and not timed_out:
                ...
                key = ("complete_with_timer" if timer_total > 0
                       else "complete_no_timer")
                qt.say(NARRATION.pick(key, name=name, score=score))
                print(f"[Score] Completed '{shape_cfg['title']}': +1 point. "
                      f"Total score: {score}")
                qt.say(NARRATION.pick("goodbye"))
                reset_round()
    finally:
